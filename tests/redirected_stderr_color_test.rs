//! otto must not write colour escape bytes into a redirected stderr.
//!
//! `colored` derives `SHOULD_COLORIZE` from `stdout().is_terminal()`
//! (`colored-3.1.1/src/control.rs`), and otto used to apply that stdout-derived
...
//!
//! `NO_COLOR` and `CLICOLOR` are removed from every child: `colored` honours
//! both, and either one exported in the developer's shell makes otto's stdout
//! plain too - at which point the zero-escape assertions hold against a binary
...
fn run(dir: &Path, home: &Path, task: &str, suffix: &str) -> Output {
