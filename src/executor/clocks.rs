//! Per-task idle timing (design doc `docs/design/2026-09-16-live-progress-renderer.md`).
//!
...
//! as still running for the rest of the run. A reader consults the registry
//! itself and uses this map only to ask how long each live task has been
//! silent.
//!
//! The ticker that used to read these clocks on an interval and print a
//! `still running` line is gone (Phase 1 of the design doc above). `TaskClock`
//! survives unmodified because the live region Phase 5 adds reads the same
//! `elapsed` / `idle_ms` this module already tracked.
