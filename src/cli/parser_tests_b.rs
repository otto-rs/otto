...
///
/// And the other direction, which is equally load-bearing: the snapshot pins
/// `[env: OTTOFILE=]` and `[env: OTTO_PROGRESS_INTERVAL=]` as well, so a future
/// test that sets EITHER of those on the process needs the `otto_progress`
/// serial key, whatever else it is about.
#[test]
