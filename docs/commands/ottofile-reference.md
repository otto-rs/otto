...
| `otto.progress-interval` | integer | `10` | **Kebab key.** Seconds of task silence before otto reports the task is still running on stderr; `0` disables. Used only when `--progress-interval` is not passed on the command line (same `-j/--jobs`-style precedence: flag, then `$OTTO_PROGRESS_INTERVAL`, then this key, then the default). See `docs/design/2026-09-15-idle-task-heartbeat.md`. |
