...
| `otto.progress` | string: `"auto"` or `"never"` | `"auto"` | Live-progress renderer mode, used only when `--progress`/`$OTTO_PROGRESS` is not passed explicitly (same `-j/--jobs`-style precedence: flag, then `$OTTO_PROGRESS`, then this key, then the default). `auto` draws a live region on stderr when it is a terminal, `TERM` is not `dumb`, and no `CI` env var is set; `never` disables it unconditionally. `"always"` is not a valid value here or on the flag; see `docs/design/2026-09-16-live-progress-renderer.md`. |
| `otto.progress-interval` | integer | `10` | **Kebab key, deprecated.** Used to gate a heartbeat line that no longer exists (removed by `docs/design/2026-09-16-live-progress-renderer.md`); accepted and ignored so an ottofile that still sets it keeps loading, and otto warns on stderr when it does. Superseded by `otto.progress`. Retained for one release, then removed. |
...
## Total: 48 fixed keys across the seven structs

`ConfigSpec` 2 + `OttoSpec` 10 + `RetentionSpec` 5 + `ForeachSpec` 9 +
`TaskSpecHelper` 13 + `ParamSpec` 7 + `EdgeSpec` 2 = **48**. This count, and
...
each struct (a compile-time trigger, since the build breaks the moment a
struct gains or loses a field, before any test even runs) and separately
recovers each struct's real on-disk key list from its "unknown field" error
message (fed a deliberately bogus key: `deny_unknown_fields`'s
derive-generated error for the six Architecture-table structs, `EdgeSpec`'s
hand-written `visit_map` error for the seventh), then asserts every one of those 48
recovered keys is mentioned, verbatim, on this page. The stated total and the
per-struct arithmetic in this section are pinned by the same test, so a
wrong count here is a red build rather than a footnote nobody re-adds up.
If this page
and the schema ever drift, `cargo test` fails: it
does not merely go stale quietly.
